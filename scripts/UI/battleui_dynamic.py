...
class BattleScreen:
    ...
    - Cell selection indicators with glow effects
    """
    ...
    def __init__(self, screen):
        ...
        self.clock = pygame.time.Clock()  # For timing animations
    ...
    def set_selected_cell(self, grid_index, row, col, is_player=True):
        """Set a specific cell as selected
        
        Args:
            grid_index (int): Which grid (0, 1, or 2)
            row (int): Row of the cell
            col (int): Column of the cell
            is_player (bool): True for player grids, False for enemy grids
        """
        grids = self.player_grids if is_player else self.enemy_grids
        if 0 <= grid_index < len(grids):
            # Don't clear all selections - just set the new one
            # This allows both player and enemy selections to show simultaneously
    ...
    def draw(self):
        ...
        # Update glow animations for all grids
        dt = self.clock.tick(60) / 1000.0  # Delta time in seconds
        for grid in self.player_grids + self.enemy_grids:
            grid.update_glow_animation(dt)
